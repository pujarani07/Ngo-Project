...
import { navArr } from "../util/home/index";
...
export default function ResponsiveNaveBar() {
  ...
  return (
    <section className="w-full bg-blue-950 lg:hidden sticky top-0 z-50 block">
        ...
        <Collapse in={isOpen} timeout="auto" unmountOnExit>
          <div className="w-full h-full bg-blue-600 text-white  ">
            {navArr.map((item) => (
              <div key={item.id}>
                <div
                  className="w-full p-4 border border-gray-200 flex items-center justify-between text-black cursor-pointer"
                  onClick={() => {
                    if (item.subMenu) {
                  ...
                  <Collapse in={openSubMenu === item.id} timeout="auto" unmountOnExit>
                    <div className="">
                      {item.subMenu.map((subItem) => (
                        <div
                          key={subItem.id}
                          className="w-full p-3 border-l border-gray-200 flex items-center justify-between text-black cursor-pointer hover:bg-gray-100"
                          onClick={() => {
